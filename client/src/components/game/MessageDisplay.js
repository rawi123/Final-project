    ...
    if (typeof card.card === "object" && card.payToPlayer) {
        return (
            <div className={`${card ? "message-card" : "message-card-hide"}`}>
                <h2>
                    {turn===currentPlayer.number?"you have to": `player ${turn+1} have to `} pay {card.payToPlayer===currentPlayer.number?"you ": card.payToPlayer?`player ${turn+1} `:null} {card.moneyTakeOut}$
                </h2>
            </div>
        )
    }
