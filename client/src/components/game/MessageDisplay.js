                    ...
                    {card.card === "prize" ? `${turn=== currentPlayer.number ? "you" : `player ${turn+1} is`} lucky! the goverment felt nice. ${turn=== currentPlayer.number ? "you" : `player ${turn+1}`} won ${card.moneyTakeOut}$` :
                        card.card === "tax" ? `Tax cover: ${turn=== currentPlayer.number ? "you" : `player ${turn+1}`} have to pay ${card.moneyTakeOut}$` :
                            card.card === "go" ? `${turn=== currentPlayer.number ? "you" : `player ${turn+1}`} got Paid 2000$` :
                                card.card === "card" ? card.rnd === 10 ? `${turn=== currentPlayer.number ? "you" : `player ${turn+1}`} Won a mysterious Pokemon!` : card.rnd <= 5 ? `${turn=== currentPlayer.number ? "you" : `player ${turn+1}`} got robbed for ${card.moneyTakeOut}$` : `A mysterious man paid ${turn=== currentPlayer.number ? "you" : `player ${turn+1}`} ${card.moneyTakeOut}$` :
